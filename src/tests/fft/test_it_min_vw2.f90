program test_it_min_vw2
! Test free energy minimization with the von Weizsäcker term handled by first
! adding it in a split-operator method (handled by fft in reciprocal space) and
! then subtracting it in the real space potential. For a given time step dt,
! the imaginary time (IT) iterations will converge towards a value. This value
! is only O(dt^4) accurate with respect to the truly converged answer with
! conjugate gradients (CG). We test a larger dt, which gives low accuracy. For
! more accurate answer, lower the dt variable, and increase the number of IT
! steps. Converged (1e-12) value seems to be achieved with dt=1e-5 and about
! 400,000 time steps. That takes a long time, so the default parameters in this
! test are for a low accuracy case, in order for it to run fast.
use types, only: dp
...
real(dp) :: T_eV, T_au, Eee, Een, Ts, Exc, Etot, Ediff, V0, mu, Etot_conv32, &
    mu_conv32, mu_Hn, dt, psi_norm, t, Hn_mu_diff, Etot_it, EvW, &
    lambda
...
if (myid == 0) then
    ...
    if (all(Ng == [32, 32, 32])) then
        ...
        call assert(abs(Etot - Etot_conv32) < 1e-11_dp)
