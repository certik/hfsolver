program test_psch3d
use types, only: dp
use constants, only: Ha2eV, density2gcm3, u2au, s2au, K2au
...
Xion(:,1) = L/2
Xion(1,1) = Xion(1,1) - 1e-4_dp ! Shift the atom, so that 1/R is defined
...
do j = 1, Ng_local(2)
do i = 1, Ng_local(1)
    R(i,j,k) = sqrt(sum((X(i,j,k,:)-Xion(:,1))**2))
end do
end do
end do
!Vloc = R**2 / 2
