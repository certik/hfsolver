program test_it_min_vw1
! Test free energy minimization with the von Weizsäcker term handled by adding
! it to the potential. The imaginary time (IT) iterations will converge towards
! a value. This value seems to always be about 1e-12 accurate with regards to
! the CG answer, irrespective of the time step dt. As such, we use a large dt
! in this test, without sacrificing accuracy.
use types, only: dp
...
!        call assert(abs(mu - mu_conv32) < 5e-8_dp)
    ...
    call assert(abs(Etot-Etot_it) < 1e-11_dp)
    call assert(abs(mu - mu_Hn) < 1e-7_dp)
end if
