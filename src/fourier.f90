...
function fft_split_radix(x) result(y)
...
if (N == 1) then
    ...
    do k = 0, N/4-1
        ...
        w2 = cos(-3*2*pi*k/N) + i_*sin(-3*2*pi*k/N)
        y(k)       = U(k)     +      (w1 * Z(k) + w2 * Zp(k))
        y(k+N/2)   = U(k)     -      (w1 * Z(k) + w2 * Zp(k))
        y(k+N/4)   = U(k+N/4) - i_ * (w1 * Z(k) - w2 * Zp(k))
        y(k+3*N/4) = U(k+N/4) + i_ * (w1 * Z(k) - w2 * Zp(k))
    end do
end if
end function
...
N = size(y)
if (N > 2) then
    call assert(size(y(:N/2-1)) == N/2)
    call assert(size(y(N/2:3*N/4-1)) == N/4)
    call assert(size(y(3*N/4:)) == N/4)
    call fft_noleaves(y(:N/2-1))
    call fft_noleaves(y(N/2:3*N/4-1))
    call fft_noleaves(y(3*N/4:))
    U  = y(:N/2-1)
    Z  = y(N/2:3*N/4-1)
    Zp = y(3*N/4:)
    do k = 0, N/4-1
...
subroutine ELAB(delta, Nl, N, Oi, Oo, S, E)
integer, intent(out) :: delta(0:)
...
if ((E .and. N == Nl) .or. (.not. E .and. N <= Nl)) then
    ...
    if (N/4 >= Nl) &
        call ELAB(delta, Nl, N/4, Oi - ishft(1, S), Oo + 3*N/4, S+2, .false.)
end if
end subroutine
