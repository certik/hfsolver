...
for n, H in enumerate(H2):
    L = Hugenholtz_index2L(H)
    print H, L
    plot_Hugenholtz(H, "graphs/mbpt2_hugenholtz%04d.png" % n)
    Gset = get_Goldstone_diagrams(H)
    Gset = [(H, G) for G in Gset]
    Gset = filter_Goldstone_diagrams(Gset)
    for n2, (H, G) in enumerate(Gset):
        holes, particles, l, h, wg, int2, en = expression(H, G, len(Gset))
        print_expression(holes, particles, l, h, wg, int2, en)
        plot_Goldstone(H, G, "graphs/mbpt2_goldstone%04d_%04d.png" % (n, n2))
    fortran(f, Gset, "term%d" % (n+1))
    f.write("\n")
    print len(Gset)
f.write("""\
end module
...
for n, H in enumerate(H3):
    L = Hugenholtz_index2L(H)
    print H, L
    plot_Hugenholtz(H, "graphs/mbpt3_hugenholtz%04d.png" % n)
    Gset = get_Goldstone_diagrams(H)
    Gset = [(H, G) for G in Gset]
    Gset = filter_Goldstone_diagrams(Gset)
    for n2, (H, G) in enumerate(Gset):
        holes, particles, l, h, wg, int2, en = expression(H, G, len(Gset))
        plot_Goldstone(H, G, "graphs/mbpt3_goldstone%04d_%04d.png" % (n, n2))
    fortran(f, Gset, "term%d" % (n+1))
    f.write("\n")
    GG.extend(Gset)
print len(GG)
...
for n, H in enumerate(H4):
    plot_Hugenholtz(H, "graphs/mbpt4_hugenholtz%04d.png" % n)
    Gset = get_Goldstone_diagrams(H)
    Gset = [(H, G) for G in Gset]
    Gset = filter_Goldstone_diagrams(Gset)
    fortran(f, Gset, "term%d" % (n+1))
    ...
    GG.extend(Gset)
print len(GG)
GG = filter_Goldstone_diagrams(GG)
print len(GG)
#for H, G in GG:
#    print H, G, get_loops(H, G)
